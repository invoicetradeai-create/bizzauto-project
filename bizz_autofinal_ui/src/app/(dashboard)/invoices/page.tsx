  ...
  const handleFormSubmit = async (invoiceData: Omit<Invoice, 'id' | 'company_id'>) => {
    try {
      if (editingInvoice) {
        ...
        await apiClient.post<Invoice>(API_ENDPOINTS.invoiceUpload, invoiceData);
                ...
                <tbody>
                  {filteredInvoices.length > 0 ? filteredInvoices.map((invoice) => (
                    <TableRow key={invoice.id} className="border-b hover:bg-muted/50">
                      ...
                      <TableCell className="p-3">{formatDate(invoice.invoice_date, "PPP")}</TableCell>
                  ...
                  )) : <TableRow><td colSpan={6} className="p-6 text-center text-muted-foreground">No invoices found.</td></TableRow>}
                </tbody>
              </table>
            </div>
          )}
