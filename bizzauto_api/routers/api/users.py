...
def read_users_me(current_user: PydanticUser = Depends(get_current_user)):
    return current_user

@router.get("/{user_id}", response_model=PydanticUser)
def read_user(user_id: UUID, db: Session = Depends(set_rls_context)):
...
def create_user_route(user: PydanticUser, db: Session = Depends(set_rls_context)):
    print("--- Creating User ---")
...
def update_user_route(user_id: UUID, user: PydanticUser, db: Session = Depends(set_rls_context)):
...
def delete_user_route(user_id: UUID, db: Session = Depends(set_rls_context)):
