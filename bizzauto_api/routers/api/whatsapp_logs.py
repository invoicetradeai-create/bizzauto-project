...
def read_whatsapp_logs(skip: int = 0, limit: int = 100, db: Session = Depends(set_rls_context)):
...
def read_whatsapp_log(whatsapp_log_id: UUID, db: Session = Depends(set_rls_context)):
...
def create_whatsapp_log_route(whatsapp_log: PydanticWhatsappLog, db: Session = Depends(set_rls_context), user: User = Depends(get_current_user)):
    return create_whatsapp_log(db=db, whatsapp_log=whatsapp_log, user_id=user.id)

@router.put("/{whatsapp_log_id}", response_model=PydanticWhatsappLog)
def update_whatsapp_log_route(whatsapp_log_id: UUID, whatsapp_log: PydanticWhatsappLog, db: Session = Depends(set_rls_context)):
...
def delete_whatsapp_log_route(whatsapp_log_id: UUID, db: Session = Depends(set_rls_context)):
