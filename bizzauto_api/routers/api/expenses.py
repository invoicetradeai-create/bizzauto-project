...
@router.get("", response_model=List[PydanticExpense])
def read_expenses(skip: int = 0, limit: int = 100, db: Session = Depends(set_rls_context), user: User = Depends(get_current_user)):
    return get_expenses(db, user_id=user.id, skip=skip, limit=limit, company_id=user.company_id)

@router.get("/{expense_id}", response_model=PydanticExpense)
def read_expense(expense_id: UUID, db: Session = Depends(set_rls_context), user: User = Depends(get_current_user)):
    db_expense = get_expense(db, expense_id=expense_id, user_id=user.id, company_id=user.company_id)
...
def create_expense_route(expense: PydanticExpense, db: Session = Depends(set_rls_context), user: User = Depends(get_current_user)):
    # Note: Ensure crud.create_expense excludes 'user_id' and 'company_id' from model_dump inside crud.py
    return create_expense(db=db, expense=expense, user_id=user.id, company_id=user.company_id)

@router.put("/{expense_id}", response_model=PydanticExpense)
@router.put("/{expense_id}/", response_model=PydanticExpense)
def update_expense_route(expense_id: UUID, expense: PydanticExpense, db: Session = Depends(set_rls_context), user: User = Depends(get_current_user)):
    db_expense = update_expense(db=db, expense_id=expense_id, expense=expense, user_id=user.id, company_id=user.company_id)
    if db_expense is None:
        raise HTTPException(status_code=404, detail="Expense not found")
    return db_expense
...
@router.delete("/{expense_id}/")
def delete_expense_route(expense_id: UUID, db: Session = Depends(set_rls_context), user: User = Depends(get_current_user)):
    db_expense = delete_expense(db=db, expense_id=expense_id, user_id=user.id, company_id=user.company_id)
    if db_expense is None:
        raise HTTPException(status_code=404, detail="Expense not found")
